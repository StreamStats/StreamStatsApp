...
app.config(['$routeProvider', function($routeProvider){
  ...
  $routeProvider.when('/beam', {
templateUrl: 'partials/beamStats.html',
controller: 'BeamStatsController',
  }).otherwise({
    templateUrl: 'partials/index.html',
    controller: 'MainController',
  });
}]);
app.controller('BeamStatsController', function($scope, $http, $sce){
    $scope.searchUser = function(){
            $http({method: 'GET', url: 'https://beam.pro/api/v1/channels/' + $scope.userSearch}).then(function successCallback(response){
         ...
         $scope.profile = $sce.trustAsHtml(html);
            });
